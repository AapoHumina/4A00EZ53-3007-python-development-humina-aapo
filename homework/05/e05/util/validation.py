...
def is_name(name):
    """
    Checks if given name argument is in a format of Firstname Lastname.

    Parameters
    ----------
    name : `string`
        name that is going to be checked it it is in name format.
    name_true : `match object`
        match object that searches for any properly capitalized name
    name_with_dashes_true : `match object`
        match object that searches for any properly capitalized name
        
    Returns
    -------
    value : `bool`
        if the name was in name format or not.
    ...
    """
    try:
        name_true = re.search("^[A-ZÖÄÅ][a-zäöå]+ [A-ZÖÄÅ][a-zäöå]+$", name)
        name_with_dashes_true = re.search("^[A-ZÖÄÅ][a-zäöå]+\-[A-ZÖÄÅ][a-zäöå]+ [A-ZÖÄÅ][a-zäöå]+$", name)

        if bool(name_true) or bool(name_with_dashes_true):
            return True
        else:
            print("Given name was not in Firstname Lastname form")
            return False
    except:
        print("Give a name in Firstname Lastname order")
        return False